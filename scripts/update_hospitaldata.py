...
def get_header(link_url):
    link_file = link_url.split('files/')[1]
    df = pd.read_csv(link_url)
    if link_file.startswith('estimated_inpatient_all'):
        df = df[['state', 'collection_date', 'Inpatient Beds Occupied Estimated',
                'Percentage of Inpatient Beds Occupied Estimated', 'Total Inpatient Beds']]
    elif link_file.startswith('estimated_inpatient_covid'):
        df = df[['state', 'collection_date', 'Inpatient Beds Occupied by COVID-19 Patients Estimated',
               'Percentage of Inpatient Beds Occupied by COVID-19 Patients Estimated']]
    else:
        df = df[['state', 'collection_date', 'Staffed Adult ICU Beds Occupied Estimated',
...
def get_data():
    ...
    link_url = link_row[0].findAll('a')[1].attrs['href']
    df1 = get_header(link_url)
    link_url = link_row[1].findAll('a')[1].attrs['href']
    df2 = get_header(link_url)
    link_url = link_row[2].findAll('a')[1].attrs['href']
    df3 = get_header(link_url)
...
files = glob.glob('time_series_data/*/hospital_data.csv')
files.sort()
